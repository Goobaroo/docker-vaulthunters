...
if ! [[ -f VaultHunters-1.13.8-Server.zip ]]; then
  rm -fr config defaultconfigs scripts forge-*.jar start.sh *Server.zip
	curl -Lo VaultHunters-1.13.8-Server.zip 'https://edge.forgecdn.net/files/3946/593/VaultHunters-1.13.8-Server.zip' && unzip -u -o 'VaultHunters-1.13.8-Server.zip' -d /data
fi
